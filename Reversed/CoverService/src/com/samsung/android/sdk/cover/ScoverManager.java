...
public class ScoverManager {
    ...
    int getCoverManagerVersion() {
        int serviceVersion = 16777216;
        if (isSupportCover()) {
            try {
                serviceVersion = ((Integer) ICoverManager.class.getMethod("getVersion", new Class[0]).invoke(getService(), new Object[0])).intValue();
            } catch (Exception e) {
    ...
    public String getServiceVersionName() {
        ...
        int revisions = sServiceVersion & 65535;
    ...
    static boolean isSupportableVersion(int supportableVersion) {
        ...
        int revisions = supportableVersion & 65535;
        int serviceMinorVersion = (sServiceVersion >> 16) & ScoverState.TYPE_NFC_SMART_COVER;
        int serviceRevisions = sServiceVersion & 65535;
    ...
    public boolean checkValidPacakge(String pkg) {
        if (!isSupportCover()) {
            ...
            try {
                ...
                if (TextUtils.isEmpty(coverAppUri) || !pkg.equals(coverAppUri.substring(1, coverAppUri.length()))) {
                    return false;
                }
                return true;
            } catch (RemoteException e) {
